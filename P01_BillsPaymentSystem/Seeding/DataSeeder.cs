...
public static class DataSeeder
{
    ...
    private static Faker<PaymentMethod> GetPaymentMethodGenerator()
    {
        ...
        .RuleFor(pm => pm.Type, f => f.Random.Enum<PaymentMethodTypes>().ToString())
        .RuleFor(pm => pm.BankAccountId, (f, pm) =>
        {
            if (pm.Type is not null && pm.Type.Equals(PaymentMethodTypes.BankAccount.ToString()))
            {
        ...
        {
            if (pm.Type is not null && pm.Type.Equals(PaymentMethodTypes.CreditCard.ToString()))
            {
    ...
    private static Faker<CreditCard> GetCreditCardGenerator()
    {
        return new Faker<CreditCard>()
            ...
            .RuleFor(cc => cc.MoneyOwed, f => f.Random.Decimal(0, 5000))
            .RuleFor(cc => cc.ExpirationDate, f => f.Date.Future(5));
    }
}